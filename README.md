...
- `plugins/runelite-plugin`: local RuneLite plugin scaffold for synthetic or live snapshot extraction
...
1. Open the Vite URL on your desktop browser to inspect the board renderer.
2. Open the same client URL on Quest Browser over LAN.
3. Use `Enter AR` to place the board on a surface once the bridge is receiving data.

Notes:

- `pnpm dev:sample` only starts the synthetic plugin sender. It does not start the bridge for you.
- Start `pnpm dev:bridge` first and wait until it is listening before starting `pnpm dev:sample`.
- `pnpm dev:sample` intentionally does not rebuild `packages/protocol`, because touching the shared package can restart the live bridge watcher and cause a connection race.
...
- synthetic mode is enabled by default
